...
- **Editor tabs (§17.2).** The tab row lists the visible files, and the sidebar lists every file. Closing a tab (its close glyph, middle-click, or Delete while it has focus) hides the file; the main pane moves to the split file, or else the next tab. Dragging a tab onto the editor shows two drop zones: the first half shows the file in the main pane, the second opens it in the split pane. Selecting the split file's tab swaps the panes, and its tab shows the columns glyph. Tabs keep the fiddle's file order and can't be reordered.
