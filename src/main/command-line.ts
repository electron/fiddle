...
import { ElectronReleaseChannel, OutputEntry, RunResult, SetupRequest } from '../interfaces';
...
async function sendTask(type: IpcEvents, task: any) {
  const onOutputEntry = (_event: Electron.IpcMainEvent, message: OutputEntry) => {
  ...
  const onTaskDone = (_event: Electron.IpcMainEvent, result: RunResult) => {
    switch (result) {
      case RunResult.SUCCESS: app.exit(0); break;
      case RunResult.FAILURE: app.exit(1); break;
      case RunResult.INVALID: app.exit(2); break;
    }
  };
