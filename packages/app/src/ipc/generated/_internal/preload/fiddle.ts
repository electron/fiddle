...
export const Palette: Partial<IPaletteRenderer> = {
  GetShowMeExamples() {
    return ipcRenderer.invoke($$ipcPrefix$$ + 'Palette_$_GetShowMeExamples');
  },
}
const $eipc_impl$__init_Palette = (localBridged: Record<string, any>) => {
  if (!(($eipc_event_validator$_MainFrame()))) return;
  localBridged['fiddle'] = localBridged['fiddle'] || {};
  localBridged['fiddle']['Palette'] = Palette
};
...
export const Run: Partial<IRunRenderer> = {
  ...
  onShowBisect(fn: () => void) {
    const handler = (e: unknown, ) => fn();
    const $$channel$$ = $$ipcPrefix$$ + 'Run_$_ShowBisect';
    ipcRenderer.on($$channel$$, handler)
    return () => { ipcRenderer.removeListener($$channel$$, handler); };
  },
}
...
export const GitHub: Partial<IGitHubRenderer> = {
  ...
  ReadClipboardToken() {
    return ipcRenderer.invoke($$ipcPrefix$$ + 'GitHub_$_ReadClipboardToken');
  },
  OpenNewTokenPage() {
  ...
  onOpenDialog(fn: (kind: GistDialogKind) => void) {
    const handler = (e: unknown, kind: GistDialogKind) => fn(kind);
    const $$channel$$ = $$ipcPrefix$$ + 'GitHub_$_OpenDialog';
    ipcRenderer.on($$channel$$, handler)
    return () => { ipcRenderer.removeListener($$channel$$, handler); };
  },
}
...
$eipc_impl$__init_Palette($$bridged$$);
