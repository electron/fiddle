...
const allChannels = { channels: ['stable', 'beta', 'nightly'] as const, showObsolete: true, showNotDownloaded: true };
...
const ids = (input: Partial<PickerInput>) => groups(input).map((g) => [g.key, g.entries.map((e) => e.label)]);

describe('pickerGroups', () => {
  // Unsorted on purpose: the picker sorts.
  const rows = [
    row('43.0.0-beta.1'),
    row('42.4.1'),
    row('43.0.0'),
  ...
  it('keeps the design groups, each newest first', () => {
    expect(ids({ rows })).toEqual([
      ...
      ['prerelease', ['44.0.0-beta.3', '43.0.0-beta.1', '43.0.0-alpha.2', '43.0.0-nightly.20260101']],
    ]);
  });

  it('drops the groups while searching: one newest-first list, nightly < alpha < beta < stable', () => {
    expect(ids({ rows, query: '43.0.0' })).toEqual([
      ['results', ['43.0.0', '43.0.0-beta.1', '43.0.0-alpha.2', '43.0.0-nightly.20260101']],
    ]);
  });
  ...
  it('puts local builds first, and matches their names', () => {
    ...
    expect(ids({ rows, localBuilds })[0]).toEqual(['local', ['gn/main - testing', 'gn/43 - release']]);
    expect(ids({ rows, localBuilds, query: '43' })).toEqual([
      [
        'results',
        ['gn/43 - release', '43.0.0', '43.0.0-beta.1', '43.0.0-alpha.2', '43.0.0-nightly.20260101'],
      ],
    ]);
  });
  ...
  it('follows the version settings but always keeps the current version', () => {
    const settings = { channels: ['stable' as const], showObsolete: false, showNotDownloaded: false };
    const installs: VersionsState['installs'] = { '42.4.1': { state: 'installed' } };
    const withObsolete = [...rows, row('30.0.0', { obsolete: true })];
    expect(ids({ rows: withObsolete, settings, installs })).toEqual([['stable', ['42.4.1']]]);
    expect(
      ids({ rows: withObsolete, settings, installs, current: { kind: 'release', version: '44.0.0-beta.3' } }),
    ).toEqual([
      ['stable', ['42.4.1']],
  ...
  it('lists versions this computer cannot run, disabled', () => {
    ...
    expect(group?.entries[0]).toMatchObject({ label: '10.0.0', state: 'unsupported', disabled: true });
  });
  ...
  it('shows install state and the design hints', () => {
    ...
    expect(byLabel['43.0.0']).toMatchObject({ id: 'r:43.0.0', state: 'installed', hint: 'latest', disabled: false });
