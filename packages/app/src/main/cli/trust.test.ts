...
function prompt(interactive: boolean, answer = ''): TrustPrompt & { ask: ReturnType<typeof vi.fn> } {
...
describe('ensureTrusted', () => {
  it('lets local fiddles and examples run without asking', async () => {
    for (const origin of [{ kind: 'local' }, { kind: 'example' }] as FiddleOrigin[]) {
      ...
      await expect(ensureTrusted({ origin, files }, {}, false, p)).resolves.toBeUndefined();
  ...
  it('lets a remote fiddle run with --trust, without asking', async () => {
    const p = prompt(false);
    await expect(ensureTrusted({ origin: gist, files }, {}, true, p)).resolves.toBeUndefined();
    expect(p.ask).not.toHaveBeenCalled();
  });
  ...
  it('asks at a terminal, showing the origin, files and dependencies', async () => {
    ...
    await expect(ensureTrusted({ origin: gist, files }, {}, false, prompt(true, 'YES '))).resolves.toBeUndefined();
    for (const answer of ['', 'n', 'no', 'sure']) {
      await expect(ensureTrusted({ origin: gist, files }, {}, false, prompt(true, answer))).rejects.toMatchObject({
        code: 'untrusted',
      });
    }
  });
