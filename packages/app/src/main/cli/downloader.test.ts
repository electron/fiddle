...
describe('fetchDownloader', () => {
  it('streams the body to the file through the given fetch, with progress', async () => {
    const fetchFn = vi.fn(async () => new Response('hello world', { headers: { 'content-length': '11' } }));
    const percents: number[] = [];
    const file = path.join(dir, 'nested', 'electron.zip');
    await fetchDownloader(fetchFn as unknown as typeof fetch).download('https://example.test/electron.zip', file, {
      getProgressCallback: (progress: { percent: number }) => percents.push(progress.percent),
    });
  ...
  it('passes the abort signal on', async () => {
    ...
    await fetchDownloader(fetchFn as unknown as typeof fetch).download('https://example.test/a', path.join(dir, 'a'), { signal });
  ...
  it('fails on an HTTP error', async () => {
    ...
    await expect(
      fetchDownloader(fetchFn as unknown as typeof fetch).download('https://example.test/b', path.join(dir, 'b'), {}),
