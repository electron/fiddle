...
const mine: ThemeFile = {
  ...
  editor: {
    base: 'hc-light',
    inherit: true,
    rules: [{ token: 'keyword', foreground: '0f4a85' }],
    colors: {},
  },
...
describe('parseTheme', () => {
  ...
  it('rejects url( in tokens, bad colours and bad JSON', () => {
    expect(
      parseTheme(
        'x',
        JSON.stringify({ ...valid, common: { accent: 'url(https://evil)' } }),
      ),
    ).toBeUndefined();
    expect(
      parseTheme(
        'x',
        JSON.stringify({ ...valid, editor: { base: 'vs', colors: { a: 'url(x)' } } }),
      ),
    ).toBeUndefined();
...
describe('themeFromMonaco', () => {
  ...
  it('works out light or dark', () => {
    ...
    expect(
      themeFromMonaco('a', { rules: [], colors: { 'editor.background': '#fafafa' } })
        .isDark,
    ).toBe(false);
    expect(
...
describe('files', () => {
  it('loads valid themes, skips invalid ones and built-in IDs, and sorts by name', async () => {
    ...
    await writeFile(
      path.join(dir, 'aurora.json'),
      JSON.stringify({ ...valid, name: 'Aurora' }),
    );
    await writeFile(path.join(dir, 'broken.json'), '{');
    await writeFile(
      path.join(dir, `${HIGH_CONTRAST_THEMES.dark}.json`),
  ...
  it('writes new theme files with a schemaVersion, and never overwrites', async () => {
    ...
    await expect(
      writeTheme(dir, 'mine', { ...mine, name: 'Other' }),
  ...
  it('round-trips a theme through its file', async () => {
    ...
    expect(await readFile(path.join(dir, 'mine-2.json'), 'utf8')).toBe(
      await readFile(path.join(dir, 'mine.json'), 'utf8'),
    );
  });

  it('makes unique, file-safe IDs that never name a built-in theme', () => {
    expect(themeId('Solarized Dark!', new Set())).toBe('solarized-dark');
    expect(themeId('Solarized Dark', new Set(['solarized-dark']))).toBe(
      'solarized-dark-2',
    );
    ...
    // Windows treats these file names as devices.
    expect(themeId('Con', new Set())).toBe('con-custom');
    expect(themeId('COM1', new Set())).toBe('com1-custom');
    expect(themeId('Console', new Set())).toBe('console');
  });
});
...
it('picks the native theme source', () => {
  ...
  expect(
    themeSource({ appearance: 'light', theme: HIGH_CONTRAST_THEMES.dark }, themes),
  ).toBe('dark');
  expect(
    themeSource({ appearance: 'dark', theme: HIGH_CONTRAST_THEMES.light }, themes),
  ).toBe('light');
});
