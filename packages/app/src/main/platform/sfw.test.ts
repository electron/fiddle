...
describe('resolveSfwEntry', () => {
  ...
  it('is undefined when the file is missing', () => {
    ...
    expect(
      resolveSfwEntry({ packaged: false, resourcesPath: '/res', resolve: noResolve }),
    ).toBeUndefined();
  });

  it('is shipped by forge.config.ts', () => {
    expect(
      fs.readFileSync(path.join(import.meta.dirname, '../../../forge.config.ts'), 'utf8'),
    ).toContain(SFW_PACKAGE_ENTRY);
  });
