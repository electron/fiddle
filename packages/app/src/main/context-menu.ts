...
export function buildContextMenuTemplate(
  ...
  const groups: MenuItemConstructorOptions[][] = [
    context === 'editor'
      ? [command('editor.goToDefinition'), command('editor.findReferences')]
      : [],
    context === 'console'
      ? [copy]
      : [
    ...
    context === 'editor'
      ? [command('editor.format'), command('editor.formatSelection')]
      : [],
...
export function attachContextMenu(
  ...
  { registry, hub, platform }: Services,
): void {
  ...
  if (isMenuDumpOn()) {
    contents.once('did-finish-load', () => {
      ...
      for (const context of focusContextSchema.options)
        dumpMenu(`context menu (${context})`, templateFor(context, params));
    });
  }
}
...
export function describeMenu(
  ...
  return template.flatMap((item) => {
    ...
    return [
      line,
      ...(Array.isArray(item.submenu) ? describeMenu(item.submenu, depth + 1) : []),
    ];
  });
}
...
function isMenuDumpOn(): boolean {
  return (
    ...
    !app.isPackaged &&
    process.env.FIDDLE_DEV_MENU_DUMP === '1'
  );
}
