...
const bisect = (
  current: string | null,
  result: { good: string; bad: string } | null = null,
) => ({
  good: '1.0.0',
  bad: '2.0.0',
  auto: true,
...
describe('finishedWindowOperations', () => {
  it('reports a finished auto-bisect', () => {
    ...
    expect(finishedWindowOperations(prev, next, undefined, 0)).toEqual([
      { kind: 'bisect', ok: true },
    ]);
    expect(finishedWindowOperations(prev, run(), undefined, 0)).toEqual([
      { kind: 'bisect', ok: false },
    ]);
  });
  ...
  it('reports package and make with their result', () => {
    ...
    expect(
      finishedWindowOperations(
        prev,
        run({ task: 'package', result: 'success' }),
        undefined,
        0,
      ),
    ).toEqual([{ kind: 'package', ok: true }]);
    expect(
      finishedWindowOperations(
        prev,
        run({ task: 'package', result: 'failure' }),
        undefined,
        0,
      ),
    ).toEqual([{ kind: 'package', ok: false }]);
  });
  ...
  it('reports only runs longer than 10 seconds', () => {
    ...
    expect(
      finishedWindowOperations(prev, run({ result: 'failure' }), 0, LONG_RUN_MS),
    ).toEqual([{ kind: 'run', ok: false }]);
    expect(
      finishedWindowOperations(prev, run({ result: 'success' }), 0, LONG_RUN_MS),
    ).toEqual([{ kind: 'run', ok: true }]);
  });
...
describe('runStarted', () => {
  it('is true only on the transition into running', () => {
    expect(runStarted(run({ status: 'starting' }), run({ status: 'running' }))).toBe(
      true,
    );
    expect(runStarted(run({ status: 'running' }), run({ status: 'running' }))).toBe(
      false,
    );
