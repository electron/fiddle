...
describe('parseKeyCombo', () => {
  ...
  it('knows the punctuation keys shortcuts use', () => {
    expect(parseKeyCombo('CmdOrCtrl+,', 'darwin')).toMatchObject({ key: ',', code: 'Comma', keyCode: 188, text: undefined });
    expect(parseKeyCombo('CmdOrCtrl+\\', 'linux')).toMatchObject({ key: '\\', code: 'Backslash', keyCode: 220 });
    expect(parseKeyCombo('CmdOrCtrl+/', 'linux')).toMatchObject({ code: 'Slash', keyCode: 191 });
    expect(parseKeyCombo('CmdOrCtrl+0', 'linux')).toMatchObject({ key: '0', code: 'Digit0', keyCode: 48 });
  });

  it('types text for Enter, Space and characters, shifted as written or as held', () => {
    expect(parseKeyCombo('Enter', 'linux')).toMatchObject({ key: 'Enter', code: 'Enter', keyCode: 13, text: '\r' });
    expect(parseKeyCombo('Space', 'linux')).toMatchObject({ key: ' ', code: 'Space', keyCode: 32, text: ' ' });
    expect(parseKeyCombo('a', 'linux')).toMatchObject({ key: 'a', keyCode: 65, modifiers: 0, text: 'a' });
    expect(parseKeyCombo('Shift+a', 'linux')).toMatchObject({ key: 'A', keyCode: 65, modifiers: shift, text: 'A' });
    expect(parseKeyCombo('A', 'linux')).toMatchObject({ key: 'A', modifiers: 0, text: 'A' });
    expect(parseKeyCombo('?', 'linux')).toMatchObject({ key: '?', code: 'Slash', text: '?' });
    expect(parseKeyCombo('Shift+/', 'linux')).toMatchObject({ key: '?', code: 'Slash', text: '?' });
  });

  it('types nothing for keys that only act', () => {
    for (const key of ['Tab', 'Escape', 'Backspace', 'Delete', 'ArrowLeft', 'Home', 'F5', 'ContextMenu']) {
      expect(parseKeyCombo(key, 'linux').text, key).toBeUndefined();
    }
    expect(parseKeyCombo('Delete', 'win32')).toMatchObject({ key: 'Delete', code: 'Delete', keyCode: 46 });
    expect(parseKeyCombo('Backspace', 'win32').keyCode).toBe(8);
    expect(parseKeyCombo('Shift+Tab', 'win32')).toMatchObject({ key: 'Tab', keyCode: 9, modifiers: shift });
    expect(parseKeyCombo('F5', 'linux')).toMatchObject({ key: 'F5', code: 'F5', keyCode: 116 });
    expect(parseKeyCombo('F12', 'linux').keyCode).toBe(123);
  });

  it('accepts DOM and Electron key names in any case, and the plus key', () => {
    expect(parseKeyCombo('ArrowDown', 'win32')).toMatchObject({ key: 'ArrowDown', keyCode: 40 });
    expect(parseKeyCombo('down', 'win32')).toMatchObject({ key: 'ArrowDown', keyCode: 40 });
    ...
    expect(parseKeyCombo('+', 'linux')).toMatchObject({ key: '+', code: 'Equal', keyCode: 187, text: '+' });
    expect(parseKeyCombo('Plus', 'linux')).toMatchObject({ key: '+', code: 'Equal', text: '+' });
    expect(parseKeyCombo('CmdOrCtrl++', 'linux')).toMatchObject({ key: '+', code: 'Equal', modifiers: control, text: undefined });
  });
  ...
  it('attaches the Edit menu commands to Cmd shortcuts on macOS only', () => {
    ...
    expect(parseKeyCombo('CmdOrCtrl+Shift+V', 'darwin').commands).toEqual(['PasteAndMatchStyle']);
  ...
  it('presses the Menu key for Shift+F10 on macOS', () => {
    expect(parseKeyCombo('Shift+F10', 'darwin')).toMatchObject({ key: 'ContextMenu', keyCode: 93, modifiers: 0, text: undefined });
    expect(parseKeyCombo('Shift+F10', 'linux')).toMatchObject({ key: 'F10', keyCode: 121, modifiers: shift });
    expect(parseKeyCombo('CmdOrCtrl+Shift+F10', 'darwin')).toMatchObject({ key: 'F10', modifiers: meta | shift });
  });
...
describe('keyForCharacter', () => {
  it('finds the key of every printable US-layout character', () => {
    ...
    expect(keyForCharacter('A')).toMatchObject({ code: 'KeyA', keyCode: 65, shiftText: 'A' });
    ...
    expect(keyForCharacter('=')).toMatchObject({ code: 'Equal', keyCode: 187, shiftText: '+' });
    expect(keyForCharacter(';')).toMatchObject({ code: 'Semicolon', keyCode: 186, shiftText: ':' });
    for (const char of '`~-_[{]}\\|\'".>,<') expect(keyForCharacter(char), char).toBeDefined();
  });
