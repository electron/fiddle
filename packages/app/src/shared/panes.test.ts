...
describe('shownPanes', () => {
  ...
  it('keeps the stored panes that are visible files, once each, up to the limit', () => {
    expect(shownPanes(['main.js', 'renderer.js'], visible, 'renderer.js')).toEqual([
      'main.js',
      'renderer.js',
    ]);
    expect(
      shownPanes(['main.js', 'gone.js', 'renderer.js', 'main.js'], visible, 'main.js'),
    ).toEqual(['main.js', 'renderer.js']);
    // One pane left after dropping unknown names: not split, so the active file shows.
    expect(shownPanes(['gone.js', 'renderer.js'], visible, 'main.js')).toEqual([
      'main.js',
    ]);
  ...
  it('puts the active file in the first pane when a stale layout does not show it', () => {
    expect(shownPanes(['preload.js', 'renderer.js'], visible, 'main.js')).toEqual([
      'main.js',
      'renderer.js',
    ]);
  });
...
describe('followActiveFile', () => {
  ...
  it('shows a newly focused file in the focused pane', () => {
    expect(
      followActiveFile(['main.js', 'renderer.js'], 'renderer.js', 'index.html', visible),
    ).toEqual(['main.js', 'index.html']);
    expect(
      followActiveFile(
        ['main.js', 'renderer.js', 'styles.css'],
        'main.js',
        'preload.js',
        visible,
      ),
    ).toEqual(['preload.js', 'renderer.js', 'styles.css']);
  });
  ...
  it('drops panes whose file was hidden or removed, and unsplits below two', () => {
    ...
    ).toEqual(['main.js', 'styles.css']);
    expect(
      followActiveFile(['main.js', 'renderer.js'], 'main.js', 'main.js', hidden),
    ).toEqual([]);
...
describe('dropOnPane', () => {
  ...
  it('shows the file in the pane it was dropped in', () => {
    expect(dropOnPane(panes, 'index.html', 1, 'center')).toEqual([
      'main.js',
      'index.html',
    ]);
  ...
  it('opens a new pane on the edge it was dropped on', () => {
    expect(dropOnPane(panes, 'index.html', 0, 'before')).toEqual([
      'index.html',
      'main.js',
      'renderer.js',
    ]);
    expect(dropOnPane(panes, 'index.html', 0, 'after')).toEqual([
      'main.js',
      'index.html',
      'renderer.js',
    ]);
    expect(dropOnPane(panes, 'index.html', 1, 'after')).toEqual([
      'main.js',
      'renderer.js',
      'index.html',
    ]);
    expect(dropOnPane(['main.js'], 'index.html', 0, 'after')).toEqual([
      'main.js',
      'index.html',
    ]);
  });
  ...
  it('moves a file that is already showing, closing its old pane', () => {
    ...
    expect(
      dropOnPane(['main.js', 'renderer.js', 'index.html'], 'index.html', 0, 'after'),
    ).toEqual(['main.js', 'index.html', 'renderer.js']);
  });
