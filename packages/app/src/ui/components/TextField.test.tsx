...
describe('TextField', () => {
  it('is labelled and reports typed text', () => {
    ...
    render(<TextField label="Gist URL" placeholder="https://gist.github.com/…" onChange={onChange} />);
  ...
  it('shows the error message and marks the input invalid', () => {
    render(<TextField label="Fiddle name" defaultValue="my fiddle!" isInvalid errorMessage="Use letters, numbers and dashes." />);
  ...
  it('connects the description', () => {
    ...
    expect(document.getElementById(describedBy.split(' ')[0]!)?.textContent).toBe('Letters, numbers and dashes.');
  });

  it('disables the input', () => {
    render(<TextField label="Electron mirror" isDisabled />);
    expect((screen.getByRole('textbox', { name: 'Electron mirror' }) as HTMLInputElement).disabled).toBe(true);
  });
});
