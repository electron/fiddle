...
describe('file operations', () => {
  ...
  it('renames, keeping content and visibility', () => {
    const fiddle = renameFile(
      renameFile(base(), 'styles.css', 'app.css'),
      'main.js',
      'main.mjs',
    );
    ...
    expect(thrownReason(() => renameFile(base(), 'main.js', 'app.js'))).toBe(
      'no-main-entry',
    );
  });
  ...
  it('keeps files in display order: sorted when created, new files last, then as moved', () => {
    const fiddle = addFile(base(), 'extra.css');
    expect(fileNames(fiddle)).toEqual([
      'main.js',
      'renderer.js',
      'index.html',
      'styles.css',
    ...
    expect(fileNames(moved)).toEqual([
      'styles.css',
      'main.js',
      'renderer.js',
      'index.html',
...
describe('VersionRefSchema', () => {
  it('parses both kinds', () => {
    expect(VersionRefSchema.parse({ kind: 'release', version: '1.0.0' })).toEqual({
      kind: 'release',
      version: '1.0.0',
    });
    expect(VersionRefSchema.parse({ kind: 'local', id: 'abc' })).toEqual({
      kind: 'local',
      id: 'abc',
    });
