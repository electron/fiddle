...
describe('pickFiddleFiles', () => {
  ...
  it('adds a main entry, but not next to a differently-cased one', () => {
    expect(pickFiddleFiles({ 'index.html': '<p/>' }).files).toEqual({
      'index.html': '<p/>',
      'main.js': '// Empty',
    });
  ...
  it.each<Record<string, string>>([
    {},
    { 'README.md': 'x', 'package.json': '{}' },
    { 'package-lock.json': '{}' },
  ])('throws when there are no supported files: %j', (map) => {
    expect(() => pickFiddleFiles(map)).toThrow(
      expect.objectContaining({
        code: ErrorCode.invalidArgument,
        details: expect.objectContaining({ reason: 'no-supported-files' }),
      }),
    );
  });
