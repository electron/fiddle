...
describe('formatText', () => {
  it('formats JavaScript, CSS and HTML in the templates’ style', async () => {
    expect(await formatText('const a = {b:"c"};', 'javascript', spaces)).toBe(
      "const a = { b: 'c' }\n",
    );
    expect(
      await formatText('a{color:red}', 'css', { tabSize: 4, insertSpaces: true }),
    ).toBe('a {\n    color: red;\n}\n');
  ...
  it('formats only the selected range', async () => {
    ...
    expect(
      await formatText(text, 'javascript', {
        ...spaces,
...
describe('registerPrettierFormatter', () => {
  ...
  it('replaces the whole model with the formatted text', async () => {
    ...
    const edits = await documents
      .get('css')!
      .provideDocumentFormattingEdits(model, spaces, token);
  ...
  it('formats a selection through the range provider', async () => {
    ...
    const edits = await ranges
      .get('javascript')!
      .provideDocumentRangeFormattingEdits(model, selection, spaces, token);
    expect(edits).toEqual([
      { range: full, text: 'const a = {b:1}\nconst c = { d: 2 }\n' },
    ]);
  });
  ...
  it('makes no edit when the text is already formatted or does not parse', async () => {
    ...
    expect(
      await documents
        .get('javascript')!
        .provideDocumentFormattingEdits(tidy, spaces, token),
    ).toEqual([]);
