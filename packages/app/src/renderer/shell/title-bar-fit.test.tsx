...
import { titleBarFit, titleBarMinWidth } from './title-bar-fit';

const PLATFORMS: readonly Platform[] = ['darwin', 'win32', 'linux'];
/** REQUIREMENTS §14: the window is usable at 600×600, its minimum size. */
const MIN_WIDTH = 600;

// @feature workspace.titlebar-fit workspace.controls
describe('titleBarFit', () => {
    ...
    for (const platform of PLATFORMS) {
      for (const menuBar of [true, false]) {
        for (const width of [600, 640, 700, 800, 1000, 1280]) {
          ...
          expect(titleBarMinWidth(platform, menuBar, fit), `${platform} at ${width} (menu bar ${menuBar})`).toBeLessThanOrEqual(width);
        }
      }
    }
    expect(titleBarMinWidth('win32', true, titleBarFit('win32', MIN_WIDTH, true))).toBe(MIN_WIDTH);
  });
  ...
  it('shows everything in the default window and gives things up only as it narrows', () => {
    ...
    for (const platform of PLATFORMS) expect(titleBarFit(platform, 1280, true)).toEqual(full);
    ...
    expect(titleBarFit('linux', MIN_WIDTH, true)).toEqual({ ...full, publishLabel: false });
    ...
    expect(titleBarFit('darwin', MIN_WIDTH, true)).toEqual({ ...full, publishLabel: false });
    ...
    expect(titleBarFit('win32', 651, true)).toEqual({ publishLabel: false, openGistButton: false, runHint: false });
  });
  ...
  it('gives things up in order: the Publish label before the Open gist button and the Run hint', () => {
    for (const platform of PLATFORMS) {
      for (let width = 1280; width >= MIN_WIDTH; width -= 4) {
        ...
        if (fit.openGistButton || fit.runHint) expect(fit.openGistButton && fit.runHint).toBe(true);
        if (!fit.openGistButton) expect(fit.publishLabel, `${platform} at ${width}`).toBe(false);
      }
    }
  });
});
