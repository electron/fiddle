...
describe('versions', () => {
  ...
  it('selects the latest stable version by default @feature versions.selection-default', async () => {
    expect((await fiddle()).versionRef).toEqual({ kind: 'release', version: '44.3.0' });
    await app().query(role('button', /^Electron 44\.3\.0\b/));
  });

  it('lists stable releases newest first, then pre-releases, and searches them @feature versions.picker versions.install-state', async () => {
    ...
    await expect
      .poll(async () => (await app().query(role('option'))).map((option) => option.name).filter((n) => n.startsWith('Electron')))
  ...
  it('copies the version number from the picker @feature versions.picker-copy', async () => {
    await app().click(picker);
  ...
  it('downloads a version from the version manager @feature versions.download settings.version-manager', async () => {
    ...
    await expect
      .poll(async () => (await appState(app())).versions?.installs['44.3.0']?.state, { timeout: 60_000 })
      .toMatch(/^(downloaded|installed)$/);
    await app().query(role('cell', 'Downloaded', { timeout: 30_000 }));
  }, 90_000);

  it("can't remove the active version @feature versions.active-undeletable", async () => {
    await app().click(role('button', /^Remove\b/, { nth: 0 }));
    await expect
      .poll(async () => `${await app().snapshot(0)}\n${(await dialogMessages(app())).join('\n')}`)
      .toContain("Electron 44.3.0 is in use, so it can't be removed.");
    expect((await appState(app())).versions?.installs['44.3.0']?.state).toMatch(/^(downloaded|installed)$/);
  });

  it('refreshes the release list on demand @feature versions.refresh', async () => {
    const count = () => app.fixtures().requests.filter((r) => r.path === '/releases.json').length;
    const before = count();
  ...
  it("switches versions, swapping an unedited template for the new version's @feature load.template-swap", async () => {
    const { fiddleRev } = await fiddle();
    await app().click(picker);
    await app().click(role('option', /^Electron 43\.7\.0\b/));
    await expect.poll(async () => (await fiddle()).versionRef).toEqual({ kind: 'release', version: '43.7.0' });
    await app().query(role('button', /^Electron 43\.7\.0\b/));
    await expect.poll(() => app.fixtures().requests.map((r) => r.path)).toContain('/minimal-repro/archive/43-x-y.zip');
    await expect.poll(async () => (await fiddle()).fiddleRev).toBeGreaterThan(fiddleRev);
  });

  it('refuses a bisect range whose good version is newer than the bad one @feature bisect.range bisect.range-order keys.bisect', async () => {
  ...
  it('bisects by hand and shows the range with its GitHub comparison @feature bisect.manual-step bisect.manual-result versions.picker-disabled', async () => {
    await app().press('CmdOrCtrl+Shift+B');
    ...
    for (let step = 0; step < 5; step++) {
      ...
      expect((await fiddle()).versionRef).toEqual({ kind: 'release', version: state.current });
    ...
    await app().query(text(`The change happened between Electron ${result?.good} and ${result?.bad}.`));
