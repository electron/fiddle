...
describe('gists', () => {
  ...
  it('refuses text that is not a gist, and opens a gist by URL while signed out', async () => {
    await openGist('not a gist');
    await app().press('Tab');
    await app().query(text('Enter a gist URL or a 32-character gist ID.'));
    await app().press('CmdOrCtrl+A', role('textbox', 'Gist URL or ID'));
    await app().type(
      `https://gist.github.com/fiddle-e2e/${FIXTURE_GIST_ID}#file-main-js`,
    );
    ...
    expect(loaded.source).toMatchObject({
      gistOwner: 'fiddle-e2e',
      gistRevision: LATEST_REVISION,
    });
  ...
  it('shows the URL of the loaded gist', async () => {
    await app().runCommand('gist.open');
    await app().query(
      text(new RegExp(`^Loaded now: https://gist\\.github\\.com/.*${FIXTURE_GIST_ID}$`)),
    );
    await app().click(role('button', 'Cancel'));
    await app().waitForAbsent(role('dialog', 'Open a gist'));
  });

  it('unlinks the gist when the fiddle is saved to a new folder', async () => {
  ...
  it('asks for a token before publishing, and checks it', async () => {
    await app().click(role('button', 'Publish'));
  ...
  it('copies an https share link for the gist', async () => {
    const { gistId } = (await fiddle()).source;
    await gistMenu('Copy share link');
    await expect
      .poll(() => app().clipboard())
      .toMatch(new RegExp(`^https://\\S+${gistId}`));
  });

  it('opens the gist dialog with its shortcut, from the gist menu and from the title bar, offering a gist link on the clipboard', async () => {
  ...
  it('updates the gist with the changed files', async () => {
    const { gistId } = (await fiddle()).source;
    ...
    const sent = JSON.parse(update?.body ?? '{}') as {
  ...
  it('lists the revisions of a gist and loads one', async () => {
    ...
    await expect
      .poll(async () => (await fiddle()).source.gistRevision)
      .toBe(FIRST_REVISION);
    expect(app.fixtures().requests.map((r) => r.path)).toContain(
      `/github-api/gists/${FIXTURE_GIST_ID}/${FIRST_REVISION}`,
    );
  });
