...
import { dialogMessages, makeFolder, readJson, role, text, useApp, windowState } from './harness.ts';
...
describe('documents', () => {
  ...
  it('shows unsaved changes in the window title @feature workspace.title-dirty files.dirty-tracking', async () => {
  ...
  it('saves to a new folder, asking for it only the first time @feature save.save keys.save save.create-folder save.gitignore files.project-name files.pkg-fields files.pkg-electron settings.package-author', async () => {
    ...
    expect(read(dir, '.gitignore').split(/\r?\n/)).toEqual(expect.arrayContaining(['node_modules', 'out']));
  ...
  it('always asks with Save As, and warns before replacing files @feature keys.save-as save.overwrite-warning', async () => {
  ...
  it('opens a folder, asking before it replaces unsaved changes @feature keys.open load.folder files.dirty-confirm', async () => {
  ...
  it('warns about an invalid package.json and still loads the folder @feature load.folder-invalid-json', async () => {
    const bad = makeFolder(app(), 'bad-json', { 'main.js': '// bad json\n', 'package.json': '{ nope' });
    ...
    await expect.poll(() => dialogMessages(app())).toContain('The fiddle loaded with warnings');
    const warning = (await app().dialogs()).find((d) => d.options.message === 'The fiddle loaded with warnings');
    expect(String(warning?.options.detail)).toContain("package.json isn't valid JSON");
  });

  it('saves a Forge project @feature save.forge-license save.forge-makers save.forge-scripts', async () => {
  ...
  it('starts a new test from the test-template branch with CmdOrCtrl+T @feature load.new-test keys.new-test', async () => {
    await app().press('CmdOrCtrl+T');
    await expect.poll(async () => (await fiddle()).source.templateName).toBe('test');
    expect(app.fixtures().requests.map((r) => r.path)).toContain('/minimal-repro/archive/test-template.zip');
  });

  it('asks before closing a window with unsaved changes @feature files.dirty-confirm keys.minimize-close', async () => {
    await edit('// unsaved');
  ...
  it('asks before quitting with unsaved changes @feature platform.quit-confirm', async () => {
    await app().queueDialog('messageBox', { button: 'Cancel' });
    await app().call('quit', {});
    await expect.poll(() => dialogMessages(app())).toContain('Quit with unsaved changes?');
    expect(await app().windows()).toHaveLength(1);
