...
/** Every file under `dir`, as `/`-separated relative paths. Empty when `dir` is missing. */
...
// Bidi controls (the ar-XB pseudo-locale) are escaped in generated source, so
...
async function compileI18n() {
  ...
  for (const locale of locales) {
    ...
    for (const ns of pseudo ? namespaces : namespacesOf(locale, localesDir)) {
      ...
      wanted.set(`${locale}/${ns}.ts`, catalogModule(messages));
...
export const pseudoLocales = ${JSON.stringify(pseudoLocales)} as const;
