...
describe('Runner options added in 3.0', () => {
  ...
  describe('signal', () => {
    it('is passed to the installer and to spawn', async () => {
      ...
      expect(spawnCall().options.signal).toBe(signal);
    });
    ...
    it('makes run() reject when aborted while the child runs', async () => {
      ...
      const run = runner.run('12.0.1', 'fiddle', { ...quiet, signal: controller.signal });
      await vi.waitFor(() => expect(child_process.spawn).toHaveBeenCalled());

      // this is what child_process does when its signal aborts
      controller.abort();
      child.emit('error', new DOMException('The operation was aborted', 'AbortError'));
