...
- On Windows and Linux, the application menu (§17.14) is a menu bar in the title bar, after the sidebar button and before the fiddle name, in the app's own style; the native menu bar is hidden (Linux) or absent (Windows, whose title bar is the app's). Main builds it per window from the same template as the native menu, so labels, keys, enablement and checked items match, and choosing an item does exactly what the native item does. When the window is too narrow, the fiddle name truncates first (and hides once only a sliver is left); then the titles that still don't fit fold, last first, into a More button at the end of the bar whose menu lists them as submenus, and when not even two titles fit beside it, the whole bar becomes one Menu button listing every menu. macOS keeps the OS menu bar. {#workspace.menubar}
- Title bar controls never overlap, at any width down to the minimum (§14): the sidebar button, menus and name on one side and Open gist, Publish and Settings on the other push the centred capsule aside rather than run under it, and before that the name truncates, the menus fold and the name hides. In the narrowest windows Publish drops to an icon (its tooltip keeps the label), then, where the platform's window controls leave too little (Windows), Open gist leaves the title bar, Run drops its key hint and the version picker narrows. {#workspace.titlebar-fit}
...
**Menu bar.** File, Edit, View, Run, Window and Help; development (unpackaged) builds add Develop before Help. macOS adds the app menu (About, Settings…, Services, Hide, Quit); on Windows and Linux, Settings… and Exit (Quit on Linux) are in File and About is in Help. Groups are separated; every item is in sentence case, with an ellipsis only on items that open a dialog asking for more. Items whose state main knows say what they will do (Hide sidebar, Stop); the command palette keeps each command's own name.
...
- View: Command palette… | Hide or Show sidebar, Hide or Show console, Split editor | Toggle soft wrap, Toggle minimap, Use Tab to move focus | Actual size, Zoom in, Zoom out | Enter or Exit full screen (the native item).
...
- Develop (development builds only, for working on Fiddle itself): Toggle title bar menu bar (checked while the bar is drawn; it shows or hides the title bar menu bar below in every window, on any platform, without a restart; on macOS the bar shows the Linux menus) | Reload, Reload all windows. The reload commands and their keys exist in every build; the toggle is a development-only command, absent from the palette and Settings > Keyboard of the packaged app.
...
**Title bar menu bar (Windows and Linux).** The menus above, drawn in the title bar (§17.1, `workspace.menubar`), behave like a Windows menu bar: a click opens a menu, a second click, Escape or a click elsewhere closes it, and while one is open, hovering another title switches to it. Left and Right move along the bar (wrapping) and take an open menu along; Down, Enter or Space open a menu on its first item, Up on its last; in a menu, Up, Down, Home and End move, a letter jumps to the next item starting with it, Right opens a submenu and Left closes it, Enter or Space choose. Alt pressed and released alone, or F10, moves focus to the bar and underlines each title's mnemonic (its first letter, or the next unused one); Alt+letter opens that menu from anywhere, inside the More or Menu button when it's folded there; Escape returns focus to where it was. The More and Menu buttons count as titles for Left, Right and hovering. Opening a menu with the mouse doesn't move focus to the title, and choosing an item restores focus before the item runs, so Cut, Copy, Paste, Select all and the editor commands act on what had it. Disabled items show disabled, checked items show a check mark, and keys are written for the platform (Ctrl+Shift+P). {#keys.menubar-alt}
