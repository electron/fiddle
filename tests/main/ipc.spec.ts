...
describe('IpcMainManager', () => {
  ...
  describe('send()', () => {
    ...
    it('does not mistake a WebFrameMain (which also has isDestroyed) for a WebContents', () => {
      // Real Electron WebFrameMain has the same `isDestroyed`/`send`
      // surface as WebContents. The discriminator has to use a
      // property that's actually unique to WebContents.
      const frame = {
        send: vi.fn(),
        isDestroyed: () => false,
